...
from .forms import RegisterForm, LoginForm, PurchasingItemForm, SellItemForm
...
@login_required
def market_page():
    purchase_form = PurchasingItemForm()
    # get the purchased item
    if request.method == "POST":
        purchased_item = request.form.get('purchased_item')
        # get th purchased item from database to perform actions on it
        ...
        if purchased_item_object:
            if current_user.can_purchase(purchased_item_object):
                ...
                db.session.commit()
                flash(f'You purchased {purchased_item_object.name}', category='success')
            else:
                flash(f"you don't have enough money to buy {purchased_item_object.name}", category='danger')
...
def register_page():
    ...
    if form.errors != {}:
        for errors_msg in form.errors.values():
            flash(f'Error: {errors_msg}', category='danger')
